...
export class AttractionsService {
  ...
  private readonly FOLDER_NAME = 'attractions';
  ...
  // -------------------------------------------------------------------------
  // ATTRACTION CATEGORY LOGIC
  // -------------------------------------------------------------------------

  /**
   * Persists a new attraction category to the database.
   * @body dto - Validated data for the new attraction category.
   * @returns The newly created attraction record.
   */
  async create(dto: CreateAttractionDto) {
    try {
      ...
      this.logger.error(
        `Failed to create attraction: ${error instanceof Error ? error.message : 'Unknown error'}`,
        error instanceof Error ? error.stack : undefined,
      );
      throw new InternalServerErrorException(
        'An unexpected error occurred while creating the attraction category.',
      );
    }
  }

  /**
   * Retrieves a paginated list of attractions with optional keyword filtering.
   * @param query - Includes search term, page number, and record limit.
   * @returns An object containing a list of attractions and pagination metadata.
   */
  async findAll(query: AttractionQueryDto) {
    const { search, page = 1, limit = 10 } = query;
    const skip = (page - 1) * limit;

    // Build the search filter conditionally
    const where: Prisma.AttractionWhereInput = search
      ? {
          OR: [
    ...
    try {
      ...
      throw new InternalServerErrorException(
        'An unexpected error occurred while searching for attractions categories.',
      );
    }
  }
   ...
   * @body dto - Validated data for the updated attraction.
  ...
  async update(id: string, dto: UpdateAttractionDto) {
    try {
      ...
      throw new InternalServerErrorException(
        'An unexpected error occurred while updating the attraction category.',
      );
    }
  }

  /**
   * Removes an attraction category and all its children
   * @param id - The ID of the attraction category to remove.
   * @returns A success message.
   */
  async remove(id: string) {
    try {
      const exists = await this.prisma.attraction.findUnique({ where: { id } });
      if (!exists) throw new NotFoundException('Attraction category not found');
      ...
      return {
        message: `Attraction category "${id}" has been successfully deleted.`,
      };
    } catch (error) {
      this.logger.error(
      ...
      throw new InternalServerErrorException(
        'An unexpected error occurred while deleting the attraction category.',
      );
    }
  }

  // -------------------------------------------------------------------------
  // ATTRACTION ITEMS (PLACES) LOGIC
  // -------------------------------------------------------------------------

  /**
   * Adds a new item/place to a category with an image upload.
   * @param attractionId - The ID of the parent attraction category.
  ...
  async addItem(
    ...
    const category = await this.prisma.attraction.findUnique({
      where: { id: attractionId },
    });
    if (!category) throw new NotFoundException('Attraction category not found');
    ...
    try {
      ...
      uploadResult = await this.cloudinary.uploadImage(
        file,
        `${this.FOLDER_NAME}/${attractionId}`,
        undefined,
      );

      // 3. Save to database
      return await this.prisma.attractionItem.create({
        data: {
          title: dto.title,
  ...
  async updateItem(
    ...
    const existingItem = await this.prisma.attractionItem.findUnique({
      where: { id: itemId },
    });
    ...
    try {
      ...
      if (file) {
        ...
        const uploadResult = await this.cloudinary.uploadImage(
          file,
          `${this.FOLDER_NAME}/${existingItem.attractionId}`,
      ...
      // 2. Update DB
      return await this.prisma.attractionItem.update({
        where: { id: itemId },
        data: {
          ...dto,
  ...
  async removeItem(itemId: string) {
    try {
      const item = await this.prisma.attractionItem.findUnique({
        where: { id: itemId },
      });
      if (!item) throw new NotFoundException('attractionItem not found');
      ...
      await this.cloudinary.deleteFolderRecursive(
        `${this.FOLDER_NAME}/${item.attractionId}/${itemId}`,
      );

      // 2. Delete from DB
      await this.prisma.attractionItem.delete({ where: { id: itemId } });

      return { message: 'Attraction item deleted successfully' };
    } catch (error) {
      this.logger.error(
  ...
  async addGalleryImages(itemId: string, files: Express.Multer.File[]) {
    ...
    try {
      ...
      return await this.prisma.attractionItemImage.createMany({
        data: uploadResults.map((res) => ({
          imageUrl: res.secure_url,
          publicId: res.public_id,
          attractionItemId: itemId,
        })),
      });
    } catch (error) {
  ...
  async getGalleryImages(itemId: string) {
    return await this.prisma.attractionItemImage.findMany({
      where: { attractionItemId: itemId },
  ...
  async removeGalleryImage(imageId: string) {
    const image = await this.prisma.attractionItemImage.findUnique({
      where: { id: imageId },
    });
    ...
    try {
      ...
      // 2. Delete from DB
      await this.prisma.attractionItemImage.delete({
        where: { id: imageId },
      });
   ...
   */
  async findAllInclusive(query: AttractionQueryDto) {
    const { search, page = 1, limit = 10 } = query;
    const skip = (page - 1) * limit;
    ...
    try {
      ...
      this.logger.error(
        `Error fetching hydrated attractions catalog: ${error instanceof Error ? error.message : 'Unknown'}`,
        error instanceof Error ? error.stack : undefined,
      );
      throw new InternalServerErrorException(
        'An unexpected error occurred while fetching the attraction catalog.',
      );
    }
  }
}
